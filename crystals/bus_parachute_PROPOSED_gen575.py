...
*** PROPOSED gen-575 (CURE-PROPOSAL, NOT LANDED) ***
Cures for gen-549 audit findings (bus msg 1783449574_762934_d434f8):
  #1 restore feed-append is now DEDUP'd by msg_id (re-run after a partial
     failure no longer duplicates feed.jsonl lines);
  #2 post refuses to overwrite an existing message file on msg_id collision
     (exclusive-create + fresh-id redraw; silent overwrite = lost message).
Land = bus/ lane owners (Hausmaster/Petrovich/Nestor). Live engine untouched.
*** END PROPOSED NOTE — everything below is original f2b60f02 + 3 hunks ***
