...
def secret(name):
    # gen-1015: dead-seat hardcode replaced by seat-portable glob (class gen-615/619)
    for p in ([os.path.expanduser("~/OMPU_shared/.secrets/"+name)]
              + sorted(_glob.glob("/sessions/*/mnt/OMPU_shared/.secrets/"+name))):
        if os.path.exists(p):
...
# gen-1015 two-layer cure (class gen-615/619, same as frontdoor gen-1014):
# layer 1 — report path derived from __file__, seat-portable, never a dead session;
# layer 2 — best-effort write: verdict above is NEVER displaced by report-write fate.
out = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "..", "errors", "publish_attempt_22_result.json")
try:
    json.dump({"ts": time.time(), "results": results,
               "dira_ship": dira_ok, "open_ship": open_ok}, open(out,"w"), indent=2)
    print(f"  raw -> {out}")
except OSError as e:
    sys.stderr.write(f"[WARN] report write failed ({e}) — verdict already printed above\n")
